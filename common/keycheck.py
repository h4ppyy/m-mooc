...
__contact__  = 'ilheeclick@gmail.com'
...
class SEED:
    ...
    def __init__(self):
        ...
        self.KC8  = 0x3779b99e
        self.KC9  = 0x6ef3733c
        self.KC10 = 0xdde6e678
        self.KC11 = 0xbbcdccf1
        self.KC12 = 0x779b99e3
    ...
    def SeedEncrypt(self, Src, RoundKey):
        ...
        R0 = []
        R1 = []
        L0.append(GetDword(Src,  0)) # L0
        L1.append(GetDword(Src,  4)) # L1
        R0.append(GetDword(Src,  8)) # R0
        R1.append(GetDword(Src, 12)) # R1
    ...
    def __SeedRound__(self, L0, L1, R0, R1, K, off):
        ...
        T1 = R1[0] ^ K[off+1]
        T1 ^= T0
        T1 = seed_SS0[GetB0(T1)] ^ seed_SS1[GetB1(T1)] ^ seed_SS2[GetB2(T1)] ^ seed_SS3[GetB3(T1)]
        T0 += T1
        T0 &= 0xFFFFFFFF
        T0 = seed_SS0[GetB0(T0)] ^ seed_SS1[GetB1(T0)] ^ seed_SS2[GetB2(T0)] ^ seed_SS3[GetB3(T0)]
        T1 += T0
        T1 &= 0xFFFFFFFF
        T1 = seed_SS0[GetB0(T1)] ^ seed_SS1[GetB1(T1)] ^ seed_SS2[GetB2(T1)] ^ seed_SS3[GetB3(T1)]
        T0 += T1
    ...
    def SeedRoundKey(self, UserKey):
        if len(UserKey) < 16: # make 16 bytes
        ...
        C = []
        D = []
        A.append(GetDword(UserKey,  0)) # A
        B.append(GetDword(UserKey,  4)) # B
        C.append(GetDword(UserKey,  8)) # C
        D.append(GetDword(UserKey, 12)) # D
        ...
        K[30] = seed_SS0[GetB0(T0)] ^ seed_SS1[GetB1(T0)] ^ seed_SS2[GetB2(T0)] ^ seed_SS3[GetB3(T0)]
        K[31] = seed_SS0[GetB0(T1)] ^ seed_SS1[GetB1(T1)] ^ seed_SS2[GetB2(T1)] ^ seed_SS3[GetB3(T1)]

        for i in range(32):
            RoundKey[i] &= 0xFFFFFFFF

        return RoundKey


    def __RoundKeyUpdate0__(self, K, off, A, B, C, D, KC):
        T0 = (A[0] + C[0] - KC) & 0xFFFFFFFF
        T1 = (B[0] + KC - D[0]) & 0xFFFFFFFF
        K[off+0] = seed_SS0[GetB0(T0)] ^ seed_SS1[GetB1(T0)] ^ seed_SS2[GetB2(T0)] ^ seed_SS3[GetB3(T0)]
        K[off+1] = seed_SS0[GetB0(T1)] ^ seed_SS1[GetB1(T1)] ^ seed_SS2[GetB2(T1)] ^ seed_SS3[GetB3(T1)]

        T0 = A[0]
        A[0] = ((A[0]>>8) ^ ((B[0] & 0xFF)<<24)) & 0xFFFFFFFF
        B[0] = ((B[0]>>8) ^ ((T0 & 0xFF)<<24)  ) & 0xFFFFFFFF
    ...
    def __RoundKeyUpdate1__(self, K, off, A, B, C, D, KC):
        ...
        T0 = C[0]
        C[0] = ((C[0]<<8) ^ (D[0]>>24)) & 0xFFFFFFFF
        D[0] = ((D[0]<<8) ^ (T0>>24)  ) & 0xFFFFFFFF
    ...
    def make_usekey_encryption(self, job, memid, usekey):

        s = SEED()

        # 1. round key test
        # usekey have fixed length(12)
        if usekey == None or len(usekey) < 12:
            usekey = s.make_usekey()

        if len(usekey) < 16:  # make 16 bytes
            usekey += '\x00' * (16 - len(usekey))

        rk = s.SeedRoundKey(usekey)

        if self.gDebug:
            print "this is roundkey : ", rk
        ...
        if memid != None:
            if len(memid) > 10:
                memid = '0000000000'
                if self.gDebug:
                    print "kotechseed128.py : id length or empty error"
        else:
            memid = '0000000000'
            if self.gDebug:
                print "kotechseed128.py : id None error"
        # id encrypto
        plan = memid
        ciph = s.SeedEncrypt(plan, rk)

        base64_enc_usekey = base64.b64encode(usekey)
        base64_enc_memid = base64.b64encode(ciph)
        ...
        if self.gDebug:
            ...
            print "plan:", plan
            print "ciph:", ciph
            print "usekey(hex):", usekey.encode('hex')
            print "plan(hex):", plan.encode('hex')
            print "--------------- return data ---------------"
            print "base64 enc_usekey:", base64_enc_usekey
            print "base64 enc_memid:", base64_enc_memid

        return [base64_enc_usekey, base64_enc_memid]
        #return [enc_usekey.encode('hex'), enc_memid.encode('hex')]


    def make_usekey_decryption(self, job, base64_enc_usekey, base64_enc_memid):
        ...
        #decode
        memid = s.SeedDecrypt(enc_memid, rk)

        if self.gDebug:
            print "dec_memid : ", memid, len(memid)
        ...
        usekey = usekey.replace('\x00', '')
        memid = memid.replace('\x00', '')
...
gDebug = False
gDebug = True
#base64 enc_usekey: MjAxODEwMDQwNDA1AAAAAA==
#base64 enc_memid: KrKe3wRAV/Ilm/HjM+Fkgw==
#localhost:18000/?usekey=MjAxODEwMDQwNDA1AAAAAA==&memid=KrKe3wRAV/Ilm/HjM+Fkgw==
if gDebug:
    if __name__ == "__main__":
        se = SEED()
        #encdata = se.make_usekey_encryption(1, '1624810', '194508151504')
        #encdata = se.make_usekey_encryption(1, '1624810', None)
        encdata = se.make_usekey_encryption(1, '1589437', None)
        ...
        decdata = se.make_usekey_decryption(1, encdata[0], encdata[1])
        if gDebug:
